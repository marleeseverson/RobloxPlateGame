...
const createEslintRule = RuleCreator((name) => {
...
function getRobloxDataTypeName(type) {
	...
	return robloxTypes.has(name) ? name : void 0;
}
...
//#endregion
//#region node_modules/.pnpm/ts-api-utils@2.4.0_typescript@5.9.3/node_modules/ts-api-utils/lib/index.js
...
function getMessageId(name) {
	if (LuauAst.LUAU_RESERVED_KEYWORDS.has(name)) return INVALID_IDENTIFIER;
	if (RESERVED_KEYWORDS.has(name)) return RESERVED_IDENTIFIER;
...
function isRestricted(name) {
	return LuauAst.LUAU_RESERVED_KEYWORDS.has(name) || RESERVED_KEYWORDS.has(name) || !LuauAst.LUAU_IDENTIFIER_REGEX.test(name);
}
function validateIdentifier(context, node, name, validate) {
	if (!isRestricted(name) || validate?.() === false) return;
	context.report({
